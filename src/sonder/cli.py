...
@click.pass_context
def run(ctx: click.Context, mode: str, ticks: Optional[int], speed: float,
        width: Optional[int], height: Optional[int], entities: int,
    ...
    try:
        ...
        for i in range(entities):
            import random
            frog = entity_registry.create("frog", 
...
def config() -> None:
    ...
    click.echo(f"Terminal size: {display_config.terminal_width}x{display_config.terminal_height}")
